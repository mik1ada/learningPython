...
def analyzeTemperatures(temperatures):
  ...
  results = (average, max(temperatures), min(temperatures))
  return results

listOfTemperatures = []

while True:
  newTemperature = input("Wprowadz temperature (jesli chcesz zakonczyc wpisywania napisz 'koniec'):")
  if newTemperature.lower() == 'koniec':
    break
  newTemperature = float(newTemperature)
  listOfTemperatures.append(newTemperature)

results = analyzeTemperatures(listOfTemperatures)
print(f"Wartosc srednia to: {results[0]}, maksymalna wartosc temperatury to: {results[1]}, a wartosc minimalna to: {results[2]}")